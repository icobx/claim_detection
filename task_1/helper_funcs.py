...
def get_word_sent_embedding(tweet, model, tokenizer):
    ...
    for token in token_embeddings:
        ...
        cat_vec = torch.cat(
            (token[-1], token[-2], token[-3], token[-4]), dim=0)
    ...
    sent_word_catavg_wostop = avg_wordembs_wostop(
        tokenized_text, token_vecs_cat, len(token_vecs_cat[0]))
    ...
    sent_word_sumavg_wostop = avg_wordembs_wostop(
        tokenized_text, token_vecs_sum, len(token_vecs_sum[0]))
    ...
    sent_emb_2_last_wostop = avg_wordembs_wostop(
        tokenized_text, token_vecs.cpu().numpy(), len(token_vecs[0]))
    ...
    sent_emb_last_wostop = avg_wordembs_wostop(
        tokenized_text, token_vecs.cpu().numpy(), len(token_vecs[0]))
