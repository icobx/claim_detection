...
def get_best_svm_model(feature_vector_train, label, feature_vector_valid, wordmod):
    ...
    for pca_nk in tqdm(pca_list, desc='pca_nk', leave=False):
        print(pca_nk)
        if pca_nk != 1.0:
            ...
            for C in tqdm(params['C'], desc='params[C]', leave=False):
                for gamma in tqdm(params['gamma'], desc='params[gamma]', leave=False):
                    ...
                    results_fpath = my_loc + \
                        '/results/task1_%s_posdep_svm_dev_%d_%d.tsv' % (
                            wordmod, args.tag_type, args.norm_type)
...
wordvec_list = ['glove-twitter-25', 'glove-twitter-50',
...
for wordmod in wordvec_list:
    ...
    if wordmod == 'spacy':
        ...
        wd_model = api.load(wordmod, return_path=False)
        sz = wordmod.split('-')[-1]
    ...
    accuracy, best_pca_nk, classifier = get_best_svm_model(
        ft_train, train_y, ft_val, wordmod)
    ...
    # classifier.save_to_file(my_loc+'/models/'+wordmod +
    #                         '_posdep_%d_%d.dt' % (args.tag_type, args.norm_type))

    with open(
        my_loc+'/models/'+wordmod +
            '_posdep_%d_%d.pkl' % (args.tag_type, args.norm_type),
        'wb'
    ) as bpcaf:
    ...
    all_res.append([wordmod, round(accuracy, 3), round(
        avg_precision, 4), best_pca_nk, ft_train.shape[1], ft_val.shape[1]])
