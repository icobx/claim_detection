        ...
        for params in param_grid:
            for C in params['C']:
                for gamma in params['gamma']:
                    ...
                    model = svm.SVC(
                        C=C, gamma=gamma, kernel=params['kernel'], random_state=42, class_weight='balanced')
                    ...
                    results_fpath = my_loc + \
                        '/results/temp5_%d.tsv' % (args.out_file)
...
train_data = json.load(open(data_path+'/proc_data/train_data2.json', 'r'))
val_data = json.load(open(data_path+'/proc_data/val_data2.json', 'r'))
...
for wordmod in wordvec_list:
    ...
    accuracy, best_pca_nk, classifier = get_best_svm_model(
        ft_train.copy(), train_y, ft_val.copy())
    ...
    results_fpath = my_loc + \
        '/results/task1_%s_pos_svm_dev_%d.tsv' % (wordmod, args.out_file)
    with open(results_fpath, "w") as results_file:
        for i, line in valDF.iterrows():
            dist = predicted_distance[i]
            results_file.write("{}\t{}\t{}\t{}\n".format('covid-19', line['tweet_id'],
    ...
    all_res.append([round(accuracy, 3), round(avg_precision, 4),
