            ...
            for C in tqdm(params['C'], desc='params[C]', leave=False):
                for gamma in tqdm(params['gamma'], desc='params[gamma]', leave=False):
                    ...
                    results_fpath = my_loc + \
                        '/results/temp9_%d.tsv' % (args.out_file)
...
train_data = json.load(
    open(data_path+'/proc_data/train_data.json', 'r'))  # train_data2.json

val_data = json.load(
    open(data_path+'/proc_data/val_data.json', 'r'))  # val_data2.json
...
for wordmod in wordvec_list:
    ...
    results_fpath = my_loc + \
        '/results/task1_%s_all_svm_dev_%d.tsv' % (wordmod, args.out_file)
    with open(results_fpath, "w") as results_file:
        for i, line in valDF.iterrows():
            dist = predicted_distance[i]
            results_file.write("{}\t{}\t{}\t{}\n".format('covid-19', line['tweet_id'],
