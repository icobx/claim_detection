...
from definitions import OUTPUT_DATA_PATH
...
def get_tweet_data(tweet_list):
    ...
    return np.array(twit_y).astype(np.int8), tweetDF
...
data = json.load(
    open(data_path+'/bert_embs/bert-large-uncased_raw_text.json', 'r'))
train_data = data['train']
val_data = data['val']
train_y = np.array(train_data['labels']).astype(np.int8)
val_y = np.array(val_data['labels']).astype(np.int8)
...
bert_list = ['sent_word_catavg_wostop',
...
for emb_type in bert_list:
    ...
    # print('ft_val', ft_val.shape)
    # print('val_pos', val_pos.shape)
    # print('val_dep', val_dep.shape)

    ft_train = np.concatenate((ft_train, train_pos, train_dep), axis=1)
    ft_val = np.concatenate((ft_val, val_pos, val_dep), axis=1)

    model_params = pickle.load(open(
        my_loc+'/models/bert-large-uncased_raw_text_%s_posdep_norm1.pkl' % (emb_type), 'rb'))
    ...
    svm_model = svm.SVC()
    with open(
        f'{my_loc}/models/bert-large-uncased_raw_text_{emb_type}'
            '_posdep_norm1.dt',
        'rb'
    ) as modelf:
...
pred_all = np.array(pred_all).astype(np.int8)
