            ...
            for C in tqdm(params['C'], desc='params[C]', leave=False):
                for gamma in tqdm(params['gamma'], desc='params[gamma]', leave=False):
                    ...
                    results_fpath = my_loc + \
                        '/results/bert_word_posdep_%s_%s_svm_norm%d.tsv' % (
...
for emb_type in emb_list:
    ...
    # print(f'ft_val - {emb_type}', ft_val.shape)
    ...
    # print('ft_val after concat', ft_val.shape)
    ...
    with open(
        my_loc+'/models/'+fname+'_'+emb_type +
            '_posdep_norm%s.pkl' % (args.normalize),
        'wb'
    ) as bpcaf:
